**LOQI - это специально созданный для данного проекта текстовый язык записи данных, хранимых [моделью предметной области](../Модель%20предметной%20области).**
...
- Т.к. LOQI был создан специально для представления [модели предметной области](../Модель%20предметной%20области) в тексте, то **он поддерживает весь функционал, предоставляемый моделью**. Поэтому для хранения, обмена, и отладки модели рекомендуется использовать именно LOQI - т.к. он полностью отображает сохраняемую в модели информацию.
	- Единственным (незначительным) исключением из этого правила являются [метаданные](../Метаданные) - поскольку в коде в них можно положить произвольные значения (т.е. любые объекты). **Запись метаданных в LOQI поддерживает только те типы данных, [](../Свойства.md#Типы%20значений%20свойств|которые%20могут%20иметь%20свойства).**
...
> [!note]
> Данный метод также принимает третий аргумент - набор так называемых опций записи. Они немного изменяют итоговый вид LOQI-текста (подробнее см. в коде), но эти изменения незначительны, и связаны с редко используемым функционалом, который мы рассмотрим [в конце данной статьи](#Свободные%20данные).
...
> - Для [классов](#Классы): `PascalCase`
> - Для [перечислений](#Перечисления%20(Enum)): `PascalCase`
> - Для значений  [перечислений](#Перечисления%20(Enum)): `MACRO_CASE`
> - Для [объектов](#Объекты): `camelCase`
> - Для [свойств](#Свойства) и [отношений](#Отношения): `camelCase`
...
Запись в LOQI использует значения тех типов, [](../Свойства.md#Типы%20значений%20свойств|которые%20могут%20иметь%20свойства).
...
Объявление [метаданных](../Метаданные) в LOQI имеет одинаковый для разных сущностей синтаксис, следующего вида:
```
[
	<свойство метаданных>*
]
...
Объявление [класса](../Классы) в LOQI имеет следующий синтаксис:
```
...
- [Объявления отношений](#Объявления%20отношений)
- [Объявления свойств](#Объявления%20свойств)
- [Утверждения о значениях свойств](#Утверждения%20о%20значениях%20свойств)
...
Объявление [объекта](../Объекты) в LOQI имеет следующий синтаксис:
```
...
- [Утверждения о связях по отношениям](#Утверждения%20о%20связях%20по%20отношениям)
- [Утверждения о значениях свойств](#Утверждения%20о%20значениях%20свойств)
...
`<вид отношения>` указывает на [](../Отношения.md#Характеристики%20отношений|соответствующую%20характеристику) данного отношения, и может иметь две формы: `<независимый вид отношения>` и `<зависимый вид отношения>`
...
```
Здесь 
- `<тип шкалы отношения>` это либо `linear` для обозначения линейной шкалы, либо `partial` для обозначения частичной шкалы. *Подробнее про шкалы [](../Отношения.md#Шкала|здесь)*
- `<квантификатор отношения>` это строка вида `{ subjCount -> objCount }`, где subjCount и objCount это либо целые числа, либо звездочка `*`. *Подробнее про квантификаторы [](../Отношения.md#Количественность%20(Квантификатор)|здесь)*
...
```
Здесь
- `<тип зависимости>` это одна из следующих строк, обозначающая соответствующий [](../Отношения.md#Типы%20зависимых%20отношений|тип%20зависимости) отношения от другого основного отношения:
	- `opposite`
	- `transitive`
	- `between`
	- `closer`
...
`<вид свойства>` это либо `class` для обозначения классового свойства, либо `obj` для обозначения объектного свойства. *Подробнее о видах свойств [](../Свойства.md#Объектные%20и%20классовые%20свойства|здесь)*
...
Объявление [перечисления](../Перечисления%20(Enum)) в LOQI имеет следующий синтаксис:
```
...
С момента их ввода уже появилась [](../Модель%20предметной%20области.md#Составление%20модели%20из%20частей|возможность%20объединять%20несколько%20различных%20моделей), что по сути приводит к тем же возможностям, в связи с чем **этот функционал практически не используется**.
