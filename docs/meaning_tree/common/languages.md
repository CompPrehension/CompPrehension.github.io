...
> [!info] 🚧 Раздел пока не написан (TODO, WIP)
> Этот раздел еще не готов. Надеемся, что он скоро станет доступен
...
Управляет работой транслятора: ограничивает его работой только с выражениями
* `true` - генерировать и парсить только одно выражение
...
Управляет режимом вывода программы (конфликтует с expressionMode=true)
Выводит всю программу или только ее точку входа (например, функцию main)
* `true` - выводить всю программу
* `false` - выводить только точку входа (например main функция/класс)
* **Используется в:** `JavaViewer`, `PythonViewer`, `CppViewer` для валидации программ
